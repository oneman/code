...
static
 char *devkeyboard = "/dev/input/by-id/usb-Dell_Dell_USB_Keyboard-event-kbd";
static
 char *devmouse = "/dev/input/by-id/usb-Logitech_USB_Optical_Mouse-event-mouse";
static
 char *devsound = "/dev/snd/by-id/usb-ZOOM_Corporation_H4essential-00";
...
void dns() {
 ...
 struct sockaddr nfo;
 nfo.sa_family = AF_INET; 
 nfo.sa_data[0] = 0;
 int br = bind(sfd, &nfo, sizeof(nfo));
 printf("hrm %d\n", br);
 exit(1);
}
